...
#define KEY_TYPE int
#define GAOP xf::database::AOP_COUNT
#define FUN count
...
void hls_db_group_aggr_cnt(hls::stream<DATA_TYPE>& in_strm,
    ...
    xf::database::groupAggregate<GAOP>(in_strm, isnull_strm, e_in_strm, out_strm, e_out_strm, kin_strm, kout_strm);
}
...
int generate_test_data(uint64_t len, std::vector<row_msg<T, KEY_T> >& testvector) {
    ...
    for (int i = 0; i < len / 2 - 1; i++) {
        ...
        for (int j = 0; j < randnum; j++) {
            testvector.push_back({rand_int(INT_RAND_MIN, INT_RAND_MAX), 0, i}); // generate random int value for data
            totalN++;
        }
    }
    n_per_group[len / 2 - 1] = rand_int(2, 7);

    for (int j = 0; j < n_per_group[len / 2 - 1]; j++) {
        testvector.push_back({NULL, 1, len / 2 - 1});
        totalN++;
    }
...
void count(hls::stream<T>& din_strm,
    ...
    for (int i = 0; i < GROUP_NUM; i++) {
        ...
        out_strm.write(ret);
        kout_strm.write(rkey);
        std::cout << "rkey, ret and isnull is " << rkey << ", " << ret << "," << isnull << std::endl;
    }
}
template <typename T, typename KEY_T>
...
int main() {
    ...
    hls::stream<bool> ref_isnull_strm("ref_isnull_strm");
    ...
    for (std::string::size_type i = 0; i < totalN; i++) {
        // print vector value
        std::cout << testVector[i].key_value << " ," << testVector[i].data << ", " << testVector[i].isnull << std::endl;
        // wirte data to stream
        kin_strm.write(testVector[i].key_value);
        din_strm.write(testVector[i].data);
        isnull_strm.write(testVector[i].isnull);
        e_in_strm.write(0);
        ref_kin_strm.write(testVector[i].key_value);
        ref_isnull_strm.write(testVector[i].isnull);
    ...
    hls_db_group_aggr_cnt(din_strm, isnull_strm, e_in_strm, dout_cnt_strm, e_out_strm, kin_strm, kout_strm);
    ...
    ref_group_aggr::FUN<DATA_TYPE, KEY_TYPE>(ref_din_strm, ref_isnull_strm, ref_dout_cnt_strm, ref_kin_strm,
                                             ref_kout_strm);
