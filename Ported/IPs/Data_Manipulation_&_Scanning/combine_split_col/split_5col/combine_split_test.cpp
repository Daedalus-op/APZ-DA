...
#define TIN ap_uint<45>
#define T1 ap_uint<6>
#define T2 ap_uint<10>
#define T3 ap_uint<3>
#define T4 ap_uint<17>
#define T5 ap_uint<9>

template <typename KEY_T1, typename KEY_T2, typename KEY_T3, typename KEY_T4, typename KEY_T5>
struct row_msg {
    KEY_T1 kin1;
    KEY_T2 kin2;
    KEY_T3 kin3;
...
void xf_database_split_col(hls::stream<TIN>& kin_strm,
                           ...
                           hls::stream<T5>& kout5_strm,
                           hls::stream<bool>& e_out_strm) {
    xf::database::splitCol(kin_strm, e_in_strm, kout1_strm, kout2_strm, kout3_strm, kout4_strm, kout5_strm, e_out_strm);
}
...
int test_function(int len) {
    ...
    hls::stream<T5> kout5_strm("kout5_strm");
    ...
    int nerror = 0;
    // ref vec
    std::vector<row_msg<T1, T2, T3, T4, T5> > refvec;
    ...
    for (std::string::size_type i = 0; i < len; i++) {
        ...
        uint64_t width_t5 = out5_value.length();
        ...
        out3_value = testVector[i].range(width_t1 + width_t2 + width_t3 - 1, width_t1 + width_t2);
        out4_value = testVector[i].range(width_in - width_t5 - 1, width_t1 + width_t2 + width_t3);
        out5_value = testVector[i].range(width_in - 1, width_in - width_t5);

        refvec.push_back({out1_value, out2_value, out3_value, out4_value, out5_value});
    ...
    e_in_strm.write(1);
    // run hls::func
    xf_database_split_col(kin_strm, e_in_strm, kout1_strm, kout2_strm, kout3_strm, kout4_strm, kout5_strm, e_out_strm);
    ...
    for (std::string::size_type i = 0; i < len; i++) {
        ...
        bool comp_res5 = (refvec[i].kin5 == out_res5) ? 1 : 0;
        if (!comp_res1 || !comp_res2 || !comp_res3 || !comp_res4 || !comp_res5) {
            nerror++;
        }
    }
    // compare e flag
