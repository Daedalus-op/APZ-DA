...
void xf_database_split_col(hls::stream<TIN>& kin_strm,
    ...
    xf::database::splitCol(kin_strm, e_in_strm, kout1_strm, kout2_strm, kout3_strm, e_out_strm);
}
...
int test_function(int len) {
    ...
    int nerror = 0;
    // ref vec
    std::vector<row_msg<T1, T2, T3> > refvec;
    ...
    for (std::string::size_type i = 0; i < len; i++) {
        ...
        out3_value = testVector[i].range(width_in - 1, width_t1 + width_t2);

        refvec.push_back({out1_value, out2_value, out3_value});
    ...
    e_in_strm.write(1);
    // run hls::func
    xf_database_split_col(kin_strm, e_in_strm, kout1_strm, kout2_strm, kout3_strm, e_out_strm);
    ...
    for (std::string::size_type i = 0; i < len; i++) {
        ...
        if (!comp_res1 || !comp_res2 || !comp_res3) {
            nerror++;
        }
    }
    // compare e flag
