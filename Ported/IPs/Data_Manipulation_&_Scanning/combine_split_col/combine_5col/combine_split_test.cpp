...
#define T5 ap_uint<12>
#define TO ap_uint<56>
...
void xf_database_combine_col1(hls::stream<T1>& kin1_strm,
                              ...
                              hls::stream<T5>& kin5_strm,
                              hls::stream<bool>& e_in_strm,
                              hls::stream<TO>& kout_strm,
                              hls::stream<bool>& e_out_strm) {
    xf::database::combineCol(kin1_strm, kin2_strm, kin3_strm, kin4_strm, kin5_strm, e_in_strm, kout_strm, e_out_strm);
}
...
template <typename KEY_T1, typename KEY_T2, typename KEY_T3, typename KEY_T4, typename KEY_T5>
void generate_test_data(uint64_t len, std::vector<row_msg<KEY_T1, KEY_T2, KEY_T3, KEY_T4, KEY_T5> >& testvector) {
    for (int i = 0; i < len; i++) {
        uint randnum = rand_uint(1, 15);
        uint randnum2 = rand_uint(1, 15);
        uint randnum3 = rand_uint(1, 15);
        uint randnum4 = rand_uint(1, 15);
        uint randnum5 = rand_uint(1, 15);
        testvector.push_back({(KEY_T1)randnum, (KEY_T2)randnum2, (KEY_T3)randnum3, (KEY_T4)randnum4, (KEY_T5)randnum5});
    }
}

// len is the data number for each T1-T2-TO group
int test_function(int len) {
    std::vector<row_msg<T1, T2, T3, T4, T5> > testVector;
    ...
    hls::stream<T5> kin5_strm("kin5_strm");
    ...
    // generate test data
    generate_test_data<T1, T2, T3, T4, T5>(len, testVector);
    ...
    for (std::string::size_type i = 0; i < len; i++) {
        ...
        kin5_strm.write(testVector[i].kin5);
        refvec.push_back(
            (testVector[i].kin1, testVector[i].kin2, testVector[i].kin3, testVector[i].kin4, testVector[i].kin5));
    ...
    e_in_strm.write(1);
    // run hls::func
    xf_database_combine_col1(kin1_strm, kin2_strm, kin3_strm, kin4_strm, kin5_strm, e_in_strm, kout_strm, e_out_strm);
