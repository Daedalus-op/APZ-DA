...
#define T2 ap_uint<6>
#define T3 ap_uint<9>
#define TO ap_uint<32>

template <typename KEY_T1, typename KEY_T2, typename KEY_T3>
struct row_msg {
    KEY_T1 kin1;
    KEY_T2 kin2;
    KEY_T3 kin3;
...
void xf_database_combine_col1(hls::stream<T1>& kin1_strm,
    ...
    xf::database::combineCol(kin1_strm, kin2_strm, kin3_strm, e_in_strm, kout_strm, e_out_strm);
}
...
template <typename KEY_T1, typename KEY_T2, typename KEY_T3>
void generate_test_data(uint64_t len, std::vector<row_msg<KEY_T1, KEY_T2, KEY_T3> >& testvector) {
    for (int i = 0; i < len; i++) {
        uint randnum = rand_uint(1, 15);
        uint randnum2 = rand_uint(1, 15);
        uint randnum3 = rand_uint(1, 15);
        testvector.push_back({(KEY_T1)randnum, (KEY_T2)randnum2, (KEY_T3)randnum3});
    }
...
int test_function(int len) {
    std::vector<row_msg<T1, T2, T3> > testVector;
    ...
    // generate test data
    generate_test_data<T1, T2, T3>(len, testVector);
    ...
    for (std::string::size_type i = 0; i < len; i++) {
        ...
        refvec.push_back((testVector[i].kin1, testVector[i].kin2, testVector[i].kin3));
    ...
    e_in_strm.write(1);
    // run hls::func
    xf_database_combine_col1(kin1_strm, kin2_strm, kin3_strm, e_in_strm, kout_strm, e_out_strm);
