...
#define TIN ap_uint<33>
#define T1 ap_uint<6>
#define T2 ap_uint<27>

template <typename KEY_T1, typename KEY_T2>
struct row_msg {
    KEY_T1 kin1;
    KEY_T2 kin2;
};
...
void xf_database_split_col(hls::stream<TIN>& kin_strm,
    ...
    xf::database::splitCol(kin_strm, e_in_strm, kout1_strm, kout2_strm, e_out_strm);
}
...
int test_function(int len) {
    ...
    int nerror = 0;
    // ref vec
    std::vector<row_msg<T1, T2> > refvec;
    ...
    for (std::string::size_type i = 0; i < len; i++) {
        ...
        out2_value = testVector[i].range(width_in - 1, width_t1);

        refvec.push_back({out1_value, out2_value});
        std::cout << "refvec data is " << refvec[i].kin1 << ", " << refvec[i].kin2 << std::endl;
        e_in_strm.write(0);
    }
    e_in_strm.write(1);
    // run hls::func
    xf_database_split_col(kin_strm, e_in_strm, kout1_strm, kout2_strm, e_out_strm);
