...
// Generate sorted tables with duplicated keys
template <typename FIELD_T, typename KEY_T>
...
// Generate sorted tables without duplicated keys
template <typename FIELD_T, typename KEY_T>
...
// pure software nested loop join as a reference
void org_merge_join(std::vector<row_msg<DATA_TYPE, KEY_TYPE> >& left_in_Vector,
...
void syn_merge_join(hls::stream<KEY_TYPE>& left_strm_in_key,
    ...
    xf::database::mergeJoin<KEY_TYPE, DATA_TYPE, DATA_TYPE>(
...
int main() {
    ...
    syn_merge_join(left_strm_key, left_strm_field, left_e_strm, right_strm_key, right_strm_field, right_e_strm,
    ...
    // run the nested loop join as a reference
    org_merge_join(left_in_Vector, right_in_Vector, left_out_Vector, right_out_Vector);
    ...
    std::cout << "" << std::endl;
    // read the data out from the stream and compare with result of nested loop join
    while (!out_e) {
