...
enum AggregateOp {
    AOP_MAX = 0,     // Max aggregation
    AOP_MIN = 1,     // Min aggregation
    AOP_MEAN = 2,    // Mean aggregation
    AOP_VARIANCE = 3,// Variance aggregation
    AOP_NORML1 = 4,  // L1 Norm aggregation
    AOP_NORML2 = 5,  // L2 Norm aggregation
    AOP_SUM = 6,     // Sum aggregation
    AOP_COUNT = 7,   // Count aggregation
    AOP_COUNTNONZEROS = 8  // Count non-zero aggregation
};
...
#include "/primary/PGSQL_Vitis/Vitis_Libraries/database/L1/include/hw/xf_database/enums.hpp"
#include "/primary/PGSQL_Vitis/Vitis_Libraries/database/L1/include/hw/xf_database/types.hpp"
#include "/primary/PGSQL_Vitis/Vitis_Libraries/database/L1/include/hw/xf_database/utils.hpp"
...
#pragma HLS pipeline
    ...
    float reti = hls::sqrt(rett);
    out_strm.write((T)reti);
    out_e_strm.write(false);
    out_e_strm.write(true);
}
} // namespace details
...
void aggregate(hls::stream<T>& in_strm,
    ...
    XF_DATABASE_STATIC_ASSERT((op == AOP_MAX) || (op == AOP_MIN) || (op == AOP_MEAN) || (op == AOP_VARIANCE) ||
                                  (op == AOP_NORML1) || (op == AOP_NORML2) || (op == AOP_SUM) || (op == AOP_COUNT) || (op == AOP_SUM)||(op == AOP_COUNT) || (op == AOP_COUNTNONZEROS),
                              "Invalid operator");
    if (op == AOP_MAX) {
